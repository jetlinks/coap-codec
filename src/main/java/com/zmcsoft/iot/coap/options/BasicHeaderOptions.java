...
import com.zmcsoft.iot.coap.exception.CoapMessageFormatException;
import com.zmcsoft.iot.coap.exception.CoapUnknownOptionException;
import com.zmcsoft.iot.coap.utils.DataConvertingUtility;
import com.zmcsoft.iot.coap.utils.HexArray;
import com.zmcsoft.iot.coap.utils.PacketUtils;
import com.zmcsoft.iot.coap.utils.StrictInputStream;
...
public class BasicHeaderOptions implements Serializable {
    ...
    int deserialize(InputStream inputStream, Code code, boolean orBlock, Optional<Integer> optionsAndPayloadLen) throws IOException, CoapMessageFormatException, PacketUtils.NotEnoughDataException {
        ...
        while (availableInternal > 0) {
            int hdrByte = PacketUtils.read8(is, orBlock);
            availableInternal--;
            ...
            if (delta == 13) {
                delta += PacketUtils.read8(is, orBlock);
                availableInternal--;
            } else if (delta == 14) {
                delta = PacketUtils.read16(is, orBlock) + 269;
                availableInternal -= 2;
            }
            if (len == 13) {
                len += PacketUtils.read8(is, orBlock);
                availableInternal--;
            } else if (len == 14) {
                len = PacketUtils.read16(is, orBlock) + 269;
                availableInternal -= 2;
            }
            headerOptNum += delta;
            byte[] headerOptData = PacketUtils.readN(is, len, orBlock);
